# Computational research scope and governance status

Recorded: 2026-09-15

Status: researcher-authored scope statement. This document is not a Boston
University human-subjects or data-governance determination.

## Scope of the work

This project is a computational machine-learning study. It trains and evaluates
models using an existing, licensed set of HRNet-derived pose annotations from
NTU RGB+D 120 and a pretrained MotionBERT checkpoint. The project does not:

- recruit participants;
- interact or intervene with participants;
- collect new participant data;
- use direct identifiers or attempt re-identification; or
- involve animals or animal-derived specimens.

All project activity is performed on computers. The source pose annotations
were nevertheless derived from recordings of people, so this project should
not be described as containing no human-derived data. Dataset access,
processing, storage, and reporting remain subject to the applicable license and
Boston University requirements.

## Approval and determination status

The scientific protocol is researcher-controlled under
[Independent research governance](../protocol/independent-research.md).
Advisor approval is not required. This does not replace an institutional
determination.

The unresolved institutional condition is recorded verbatim from protocol-v1:

> BU provides the applicable human-subjects/data-governance determination.

Until that determination is provided by the appropriate Boston University
authority, this statement documents the researcher's understanding of the
project scope but does not resolve or waive the institutional condition.
