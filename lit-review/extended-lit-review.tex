...
\usepackage[letterpaper,left=0.72in,right=0.72in,top=0.68in,bottom=0.68in]{geometry}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{navy}{Fall 2026 UROP Application}}
\fancyhead[R]{\small\textcolor{navy}{Giacomo Cappelletto}}
...
\titleformat{\section}
  ...
  {}{0pt}{}
  [\vspace{-0.35em}\textcolor{teal}{\rule{\textwidth}{0.55pt}}]
...
% FIELD_START: title
{\sffamily\bfseries\color{navy}\fontsize{20}{22}\selectfont
Robust Human-Motion Retrieval\\[-0.08em]
from Noisy Pose Sequences}\par
% FIELD_END: title
\vspace{0.35em}
{\sffamily\large Giacomo Cappelletto}\hfill
{\sffamily\color{slate}Faculty Mentor: Professor Brian Kulis}\par
{\sffamily\color{slate}Boston University College of Engineering}\hfill
{\sffamily\color{slate}Final proposal - August 16, 2026}\par
\vspace{0.45em}
\colorbox{mist}{\parbox{\dimexpr\textwidth-2\fboxsep\relax}{%
\small\textbf{Research question.} Given one unmodified example of each new action, does contextual training help a model find the correct match when body joints or video frames are missing or inaccurate?}}
...
\begin{itemize}
  \item \textbf{Weeks 1-2 - Data and protocol:} obtain approved NTU files, compatible poses, official class split, and reference list; freeze validation and test episodes; add leakage checks.
  \item \textbf{Weeks 3-4 - Evaluator and stress tests:} validate feature extraction and clean retrieval; implement metrics and deterministic joint-jitter, joint-masking, and frame-removal tests.
  \item \textbf{Weeks 5-7 - Matched training:} train the three output layers with identical controls; select settings only on unmodified validation data and resolve unstable runs.
  \item \textbf{Weeks 8-9 - Repeated evaluation:} run three paired seeds and all prespecified corruption severities; record clean and corrupted accuracy, time, memory, and failed runs.
  \item \textbf{Weeks 10-11 - Analysis:} calculate accuracy losses and confidence intervals, inspect errors, compare corruption types, and run only essential ablations.
  \item \textbf{Weeks 12-13 - Finalization:} reproduce figures from saved configurations, document permitted code, and prepare the final report and UROP presentation or poster.
\end{itemize}
