    ...
    protocol = protocol.model_copy(
        update={
            "dataset": protocol.dataset.model_copy(
                update={"expected_source_sample_count": len(inventory.records)}
            )
        }
    )
...
def test_data_generate_cli_writes_the_verified_bundle(
    protocol_path: Path,
    tmp_path: Path,
    ...
    assert printed["files"]["source-inventory.jsonl"]["records"] == 180
    assert (output_dir / "manifest-audit.md").is_file()
